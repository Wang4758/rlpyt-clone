...
class MinibatchRlBase(BaseRunner):
    ...
    def log_diagnostics(self, itr, traj_infos=None, eval_time=0, prefix='Diagnostics/'):
        ...
        logger.dump_tabular(with_prefix=False, no_terminal_output=True)
...
class MinibatchRl(MinibatchRlBase):
    ...
    def train(self):
        ...
        logger.log('MinibatchRL: Startup...')
        n_itr = self.startup()
        logger.log('MinibatchRL: Startup done.')
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                self.save_itr_snapshot(itr) # call every itr otherwise doesnt work well with the gap saving...
                if (itr + 1) % self.log_interval_itrs == 0:
                    self.log_diagnostics(itr)
        training_duration = (time.time() - start_time) / 60.0
        logger.log(f'Training took {round(training_duration, 1)} minutes')
        self.shutdown()
