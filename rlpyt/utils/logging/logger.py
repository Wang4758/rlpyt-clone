...
def set_variant_base_dir(exp_dir):
    global _variant_base_dir
    _variant_base_dir = exp_dir

def get_variant_base_dir():
    global _variant_base_dir
    return _variant_base_dir
...
def dump_tabular(no_terminal_output = False, *args, **kwargs):
    if not _disabled:  # and not _tabular_disabled:
        wh = kwargs.pop("write_header", None)
        if len(_tabular) > 0:
            if _log_tabular_only and not no_terminal_output:
