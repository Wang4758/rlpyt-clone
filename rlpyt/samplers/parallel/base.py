...
class ParallelSamplerBase(BaseSampler):
    ...
    def initialize(
        ...
        is_commonroad = ('commonroad' in self.env_kwargs['id']) or ('cr-monitor' in self.env_kwargs['id'])
        if is_commonroad:
            env = self.EnvCls(**self.env_kwargs, rlpyt_fast_init=True)
        else:
    ...
    def _assemble_common_kwargs(self, affinity, global_B=1):
        ...
        if self.eval_n_envs > 0:
            common_kwargs.update(dict(
                ...
                eval_n_envs_global=self.eval_n_envs,
    ...
    def _assemble_workers_kwargs(self, affinity, seed, n_envs_list):
        ...
        for kwa in workers_kwargs:
            kwa.update(dict(world_size=g_env))
