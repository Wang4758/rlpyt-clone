...
def sampling_process(common_kwargs, worker_kwargs):
    ...
    if ('commonroad' in c.env_kwargs['id']) or ('cr-monitor' in c.env_kwargs['id']):
        envs = [
            c.EnvCls(**c.env_kwargs, mp_rank=env_rank, mp_world_size=global_B)
            for env_rank, global_B in zip(w.env_ranks, repeat(c.global_B))
        ]
    else:
    ...
    if c.get("eval_n_envs", 0) > 0:
        if ('commonroad' in c.env_kwargs['id']) or ('cr-monitor' in c.env_kwargs['id']):
            eval_rank_worker_offset = w.rank * c.eval_n_envs
            eval_env_ranks = [i + eval_rank_worker_offset for i in range(c.eval_n_envs)]
            eval_env_world_size = c.eval_n_envs_global
            eval_envs = [
                c.EnvCls(**c.eval_env_kwargs, mp_rank=env_rank, mp_world_size=global_B)
                for env_rank, global_B in zip(eval_env_ranks, repeat(eval_env_world_size))
            ]
        else:
